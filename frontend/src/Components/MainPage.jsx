                    ...
                    <ul className="todo-list">
                        {todos.map(todo=>{ return(
                            <li className="todo my-3" key={todo.id}>
                                <input type="checkbox" value={todo.id} onChange={updateTodo} checked={todo.doneStatus}/>{todo.todoText}<button onClick={deleteTodo} className="mx-3 btn btn-primary" value={todo.id}>Delete</button>
                            </li>
                        )})}
                    </ul>
                </div>
